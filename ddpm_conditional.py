...
config = SimpleNamespace(    
    ...
    img_size = 64,
    num_classes = 10,
    dataset_path = r"C:\Users\wlc\PycharmProjects\MRes_Pipeline\data\DTI_LGE_paired",
    train_folder = "train",
    val_folder = "test",
    device = "cuda",
    slice_size = 1,
...
class Diffusion:
    ...
    def sample(self, use_ema, conditions, cfg_scale=3):
    ...
    def one_epoch(self, train=True):
        ...
        for i, train_data in enumerate(pbar):
            images, conditions = train_data['B'], train_data['A']
            with torch. torch.cuda.amp.autocast() and (torch.inference_mode() if not train else torch.enable_grad()):
                ...
                if np.random.random() < 0.1:
                    conditions = None
                predicted_noise = self.model(x_t, t, conditions)
    ...
    def log_images(self):
        ...
        val_data = self.val_dataloader.next()
        conditions = val_data['A'].to(self.device)
        sampled_images = self.sample(use_ema=False, conditions=conditions)
        wandb.log({"sampled_images":     [wandb.Image(img.squeeze().cpu().numpy()) for img in sampled_images]})

        # EMA model sampling
        ema_sampled_images = self.sample(use_ema=True, conditions=conditions)
        plot_images(sampled_images)  #to display on jupyter if available
        wandb.log({"ema_sampled_images": [wandb.Image(img.squeeze().cpu().numpy()) for img in ema_sampled_images]})
